...
UtaxiApp::UtaxiApp(string _locs)
{
	read_input_file(_locs);
}
...
vector<vector<string>> UtaxiApp::present_all_trips(string name, bool cost_sort)
{
	...
	if(user_i == -1) throw Undefined_EX();
	if(!users[user_i]->are_you_driver()) throw Access_EX();
	if(trips.size() == 0) throw Empty_EX();
...
void UtaxiApp::read_input_file(string file_address)
{
	string line;
...
Location* UtaxiApp::read_line(string line)
{
	string name;
